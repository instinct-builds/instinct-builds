// MUEW 0.49.0: straight chord arp, then a pitched octave motif with ratchets.
#include "synth.h"
...
int main(int argc,char** argv){
 const char* path=argc>1?argv[1]:"out/MUEW-0.49.0-arp-octave-demo.wav";
 ...
 for(int scene=0;scene<4;++scene){
   ...
   if(scene%2){v.arpPatOctave[0]=1;v.arpPatOctave[1]=-1;v.arpPatOctave[2]=1;
 ...
 printf("MUEW octave demo: straight / pitched+ratchet / straight / pitched+ratchet; %.2fs peak %.3f; %s\n",out.size()/(2.0*sr),peak,path);return 0;
}
