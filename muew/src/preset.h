...
//                  0.60.0 adds optional `noisebsync <division>`; zero remains free milliseconds.
//                  0.63.0 adds optional `noisebvel <depth>`; zero preserves legacy static noise.
...
struct Preset {
    ...
    std::string serialize() const {
              ...
              << voice.osc2UniDetune << " " << voice.uniWidth << " " << voice.uniBlend << "\n";
        {
            ...
            if (v.noiseBurstSync != d.noiseBurstSync) o << "noisebsync " << v.noiseBurstSync << "\n";
            ...
            if (v.noiseBurstVelocity != d.noiseBurstVelocity) o << "noisebvel " << v.noiseBurstVelocity << "\n";
            if (v.noiseBurstVelTime != d.noiseBurstVelTime) o << "noisebvtime " << v.noiseBurstVelTime << "\n";
            if (v.noiseBurstKeyTime != d.noiseBurstKeyTime) o << "noisebkey " << v.noiseBurstKeyTime << "\n";
            if (v.noiseBurstVelColor != d.noiseBurstVelColor) o << "noisebvcolor " << v.noiseBurstVelColor << "\n";
    ...
    bool parse(const std::string& text) {
        ...
        *this = Preset{};
        ...
        while (std::getline(in, line)) {
            ...
            else if (key == "noisebsync") {
                int division; if (ls >> division) voice.noiseBurstSync = std::clamp(division, 0, kSyncCount - 1);
            }
            ...
            else if (key == "noisebvel") {
                double depth; if (ls >> depth && std::isfinite(depth)) voice.noiseBurstVelocity = std::clamp(depth, 0.0, 1.0);
            }
            else if (key == "noisebvtime") {
                double depth; if (ls >> depth && std::isfinite(depth)) voice.noiseBurstVelTime = std::clamp(depth, 0.0, 1.0);
            }
            else if (key == "noisebkey") {
                double depth; if (ls >> depth && std::isfinite(depth)) voice.noiseBurstKeyTime = std::clamp(depth, -1.0, 1.0);
            }
            else if (key == "noisebvcolor") {
                double depth; if (ls >> depth && std::isfinite(depth)) voice.noiseBurstVelColor = std::clamp(depth, 0.0, 1.0);
            }
            else if (key == "filter2") {
    ...
    bool operator==(const Preset& o) const {
        ...
        bool voiceEq = a.osc1Shape == b.osc1Shape && a.osc2Shape == b.osc2Shape
            ...
            && a.noiseLevel == b.noiseLevel && a.noiseTone == b.noiseTone && a.noiseCharacter == b.noiseCharacter && a.noiseColor == b.noiseColor && a.noiseWidth == b.noiseWidth && a.noiseBurst == b.noiseBurst && a.noiseBurstSync == b.noiseBurstSync && a.noiseBurstAttack == b.noiseBurstAttack && a.noiseBurstCurve == b.noiseBurstCurve && a.noiseBurstVelocity == b.noiseBurstVelocity && a.noiseBurstVelTime == b.noiseBurstVelTime && a.noiseBurstKeyTime == b.noiseBurstKeyTime && a.noiseBurstVelColor == b.noiseBurstVelColor
