// MUEW 0.43.0: copied spectral profile blends into a destination range with untouched edges.
#include "synth.h"
...
int main(int argc, char** argv) {
    const char* path = argc > 1 ? argv[1] : "out/MUEW-0.43.0-spectral-profile-demo.wav";
    ...
    printf("MUEW profile demo: original / falloff edge / pasted center / original; %.2f s, peak %.3f; %s\n", out.size() / (2.0 * sr), peak, path);
    return 0;
}
