// MUEW 0.41.0: an original wavetable phrase, with H4-H8 painted in its middle frames.
#include "synth.h"
...
int main(int argc, char** argv) {
    const char* path = argc > 1 ? argv[1] : "out/MUEW-0.41.0-harmonic-brush-demo.wav";
    ...
    FrameRange range{4, 11};
    PartialBrush brush;
    brush.line(4, -9, 8, -19);
    if (!applyBrushTable(frames, range, 7, brush)) return 1;
    if (frames[0] != before[0] || frames[15] != before[15] || frames[7] == before[7]) return 2;
    ...
    printf("MUEW brush demo: outside / painted / painted / outside; %.2f s, peak %.3f; %s\n", out.size() / (2.0 * sr), peak, path);
    return 0;
}
