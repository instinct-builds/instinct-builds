// MUEW 0.44.0: copied H4-H16 profile blends into a destination frame range.
#include "synth.h"
...
int main(int argc, char** argv) {
    const char* path = argc > 1 ? argv[1] : "out/MUEW-0.44.0-spectral-span-demo.wav";
    ...
    printf("MUEW span demo: original / range edge / H4-H16 center / original; %.2f s, peak %.3f; %s\n", out.size() / (2.0 * sr), peak, path);
    return 0;
}
