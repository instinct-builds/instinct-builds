...
set -euo pipefail
cd "$(dirname "$0")/.."
rm -rf out/MUEW.app out/MUEW-0.3.0.dmg out/MUEW-distribution
mkdir -p out/MUEW.app/Contents/MacOS out/MUEW.app/Contents/Resources
SDK=$(xcrun --show-sdk-path)
clang++ -std=c++17 -O2 -fobjc-arc -arch arm64 -arch x86_64 -isysroot "$SDK" -Isrc \
  app/MUEWStandalone.mm -framework AppKit -framework AVFoundation -framework AudioToolbox \
...
hdiutil create -volname MUEW -srcfolder out/MUEW-distribution -ov -format UDZO out/MUEW-0.3.0.dmg
