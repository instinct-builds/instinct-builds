...
- (void)drawArpPattern {
    ...
    for (int i = 0; i < arp::kPatSteps; ++i) {
        ...
        if (used && kind == arp::StepOn) {
            const int oct = std::clamp(v.arpPatOctave[i], -1, 1);
            if (oct) {
                FillRound(NSMakeRect(c.origin.x + 1, c.origin.y + 10, c.size.width - 2, 8), 2, C(0x243440));
...
- (NSRect)burstBar:(int)i { NSRect r = [self burstPanel]; return NSMakeRect(r.origin.x + 22, NSMaxY(r) - 76 - i * 19, 176, 15); }
- (NSRect)burstPlot { NSRect r = [self burstPanel]; return NSMakeRect(r.origin.x + 218, r.origin.y + 38, 186, 114); }
- (NSRect)burstSyncRect { NSRect r = [self burstPanel]; return NSMakeRect(r.origin.x + 22, r.origin.y + 47, 176, 17); }
...
- (void)drawTableEditor {
    ...
    if (wtMode == 3) { // 0.31.0 SPECTRAL: preview of the processed frame over the original, bars, APPLY / RESET
        ...
        if (wtPartialLarge) {
            const NSRect taper = [self wtTaperBar];
            TextA(@"EDGE", NSMakeRect(pv.origin.x + 5, taper.origin.y - 2, 31, 10), 6.5,
                  wtRange.active((int)t.size()) ? col : C(0x56616d), NSFontWeightBold, NSTextAlignmentLeft);
            FillRound(taper, 3, C(0x26313c));
            FillRound(NSMakeRect(taper.origin.x, taper.origin.y, std::max<CGFloat>(2, taper.size.width * wtBrushTaper), taper.size.height),
                      3, [col colorWithAlphaComponent:.8]);
            FillRound(NSMakeRect(taper.origin.x + taper.size.width * wtBrushTaper - 3, taper.origin.y - 2, 6, 10),
    ...
    if (wtRange.active(nT)) {
        ...
        if (wtMode == 3 && wtPartialLarge) for (int i = 0; i < 16; ++i) {
            const NSRect r = [self wtThumb:i];
            const int frame = ThumbFrame(i, nT);
            if (!wtRange.contains(frame, nT)) continue;
            const double strength = brushRangeStrength(frame, wtRange, nT, wtBrushTaper);
            FillRound(NSMakeRect(NSMidX(r) - 8, NSMaxY(r) + 7, 16 * strength, 2), 1, [col colorWithAlphaComponent:.8]);
        }
    }
    for (int i = 0; i < 16; ++i) {
...
- (void)drawRect:(NSRect)dirty {
    ...
    if (filterPage == 2) {
        ...
        NSRect width = [self noiseWidthRect];
        FillRound(width, 3, C(0x1c232d));
        if (v.noiseWidth > 0) FillRound(NSMakeRect(width.origin.x, width.origin.y, width.size.width * v.noiseWidth, width.size.height), 3, C(0x95b8e5, .58));
...
- (void)drawBurstDetail {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        const double norm = i == 3 ? current.voice.noiseBurstVelTime : i == 2 ? current.voice.noiseBurstVelocity : i ? (current.voice.noiseBurstCurve + 1) * .5 : current.voice.noiseBurstAttack / .8;
        FillRound(r, 4, C(0x10161d));
        FillRound(NSMakeRect(r.origin.x, r.origin.y, r.size.width * norm, r.size.height), 4, C(0xf5cc78, .52));
        NSString* name = i == 3 ? @"VEL TIME" : i == 2 ? @"VEL DEPTH" : i ? @"DECAY CURVE" : @"ATTACK";
        NSString* value = i == 3 ? [NSString stringWithFormat:@"%.0f%%", current.voice.noiseBurstVelTime * 100] : i == 2 ? [NSString stringWithFormat:@"%.0f%%", current.voice.noiseBurstVelocity * 100] : i ? [NSString stringWithFormat:@"%+.2f", current.voice.noiseBurstCurve] : [NSString stringWithFormat:@"%.0f%%", current.voice.noiseBurstAttack * 100];
    ...
    for (int k = 0; k <= 64; ++k) {
        ...
        const double timeFactor = 1.0 - current.voice.noiseBurstVelTime * .4; // example note at 60% velocity
        const float y = noiseBurstGain((uint64_t)std::floor(k / timeFactor), 64,
                                       current.voice.noiseBurstAttack, current.voice.noiseBurstCurve) *
                        (1.0f - (float)current.voice.noiseBurstVelocity * .4f);
...
- (BOOL)burstDetailMouseDown:(NSPoint)p event:(NSEvent*)e {
    ...
    for (int i = 0; i < 4; ++i) if (NSPointInRect(p, NSInsetRect([self burstBar:i], -3, -3))) {
        NSRect r = [self burstBar:i];
        const double x = std::clamp((p.x - r.origin.x) / r.size.width, 0.0, 1.0);
        if (i == 3) current.voice.noiseBurstVelTime = e.clickCount == 2 ? 0.0 : std::round(x * 100) / 100.0;
...
- (void)wtTaperTo:(NSPoint)p {
    const NSRect b = [self wtTaperBar];
    wtBrushTaper = std::round(std::clamp((p.x - b.origin.x) / b.size.width, 0.0, 1.0) * 20) / 20.0;
    [self setNeedsDisplay:YES];
}
...
- (void)wtBrushTo:(NSPoint)p {
    ...
    wtBrushChanged = applyBrushTable(next, wtRange, wtFrame, wtBrush, wtBrushTaper);
    if (wtBrushChanged) {
        current.tables[wtEdit] = std::move(next);
        edited = true; [self applySound];
    } else if (current.tables[wtEdit] != wtBrushBase) {
        current.tables[wtEdit] = wtBrushBase;
        [self applySound];
    }
    [self setNeedsDisplay:YES];
}
...
- (void)wtPartialGain:(int)db {
    ...
    if (wtFrame < 0 || wtFrame >= (int)t.size()) return;
    TableFrames next = t;
    bool changed = wtRange.active((int)t.size()) ? gainPartialRange(next, wtRange, wtPartial, db)
        : gainPartial(next[wtFrame], wtPartial, db);
    if (!changed) return;
    [self wtRemember:wtRange.active((int)t.size()) ? "PARTIAL RANGE" : "PARTIAL"];
    t = std::move(next); edited = true; [self applySound]; [self setNeedsDisplay:YES];
}
...
- (void)tableMouseDown:(NSPoint)p event:(NSEvent*)event {
    ...
    if (wtMode == 3 && NSPointInRect(p, NSInsetRect([self wtCanvas], -4, -4))) { // 0.31.0 SPECTRAL page
        ...
        if (wtPartialLarge) {
            if (wtRange.active((int)t.size()) && NSPointInRect(p, NSInsetRect([self wtTaperBar], -5, -7))) {
                wtTaperDrag = 0; [self wtTaperTo:p]; return;
            }
            for (int i = 0; i < 4; ++i)
