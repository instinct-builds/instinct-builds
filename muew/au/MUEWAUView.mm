...
@interface MUEWAUEditorContainer_0_63 : MUEWEditorView {
@public
    AUEditorHost* auHost;
    NSTimer* follow;
}
...
@implementation MUEWAUEditorContainer_0_63
...
- (void)muewFollowPerformance {
    ...
    [self showOutputLeft:pf.outputPeak[0] right:pf.outputPeak[1] drive:pf.masterDrive]; // 0.58.0
    [self showRouteMeters:pf.routeMeter count:16]; // 0.54.0
    [self showRouteMin:pf.routeMin max:pf.routeMax count:16];
...
- (void)viewDidMoveToWindow {
    ...
    if (self.window && auHost) {
        __weak MUEWAUEditorContainer_0_63* weakSelf = self;
...
@interface MUEWViewFactory_0_63 : NSObject <AUCocoaUIBase>
@end

@implementation MUEWViewFactory_0_63
...
- (NSView*)uiViewForAudioUnit:(AudioUnit)inAudioUnit withSize:(NSSize)inPreferredSize {
    ...
    MUEWAUEditorContainer_0_63* v = [[MUEWAUEditorContainer_0_63 alloc] initWithFrame:NSMakeRect(0, 0, 1000, 680)];
